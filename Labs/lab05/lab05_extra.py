...
def build_successors_table(tokens):
    ...
    for word in tokens:
        if prev not in table:
            table[prev] = [tokens[0]] if prev == '.' else [word]
        else:
            table[prev] += [word]
        prev = word
    return table
...
def add_trees(t1, t2):
    ...
    # when two trees have the same number of branches, adds up the labels and return branches added
    if len1 == len2:
        return tree(label(t1) + label(t2), [add_trees(b1, b2) for b1, b2 in zip(branches(t1), branches(t2))])
    # since zip is only useful when called on same length, when branches are not the same,
    # I add a subtree with label=0
    elif len_1 < len_2:
