...
def prune_leaves(t, vals):
    ...
    if is_leaf(t) and label(t) in vals:
        ...
        return tree(label(t), [b for b in [prune_leaves(b, vals) for b in branches(t)] if b is not None])
...
def balanced(m):
    ...
    False
    """
    if is_weight(m):  # if m is a weight, return True
        return True
    else:  # m is a mobile
    ...
    9
      ...
      6
        ...
        5
          ...
          2
    """
    if is_weight(m):  # when m is a weight, return tree(size(m))
        return tree(size(m))
    else:  # when m is a mobile, recursively call itself
